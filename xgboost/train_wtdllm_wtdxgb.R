...
data_full_llm_xgb <- data_full_llm_xgb_raw %>% 
  mutate(
...
### XGBoost sur pred --------------------------------------------------------------------
# Test boboche
xgb_llm_xgb_s_1 <- xgboost(
  data = dmtrain_llm_xgb_s_full, # the data
...
prob_llm_xgb_s_1 <- predict(xgb_llm_xgb_s_1, dmvalid_llm_xgb_s_full)
prob_llm_xgb_s_1_mat <- matrix(prob_llm_xgb_s_1, nrow = 5)
prob_llm_xgb_s_1_mat <- t(prob_llm_xgb_s_1_mat)  # Transpose to align with rows
pred_llm_xgb_s_1 <- max.col(prob_llm_xgb_s_1_mat) - 1

colnames(prob_llm_xgb_s_1_mat) <- c("prob_rating_1", "prob_rating_2", "prob_rating_3", "prob_rating_4", "prob_rating_5")
res_llm_xgb_s_1 <- bind_cols(
  tibble("rating" = y_valid_llm_xgb+1), 
  "weighted_prediction" = NA, 
  tibble("predicted_llm_rating" = pred_llm_xgb_s_1+1), 
  as_tibble(prob_llm_xgb_s_1_mat)
)

metrics_llm_xgb_s_1 <- compute_metrics(y_valid_llm_xgb, pred_llm_xgb_s_1)





test_results <- res_llm_xgb_s_1 %>% 
